...
3. `pipeline/uploadtos3.py` uploads `data/parsed/` to `s3://enron-org-memory-data/emails/` using 64-way concurrency and skips keys that already exist in S3.
...
- AWS defaults assumed across docs/scripts: `us-west-2`, Lambda `python3.11`, 30s timeout, 256 MB memory, permissive CORS for demo API.
